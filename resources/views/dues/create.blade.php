                    ...
                    <div class="bg-dark bg-opacity-50 rounded-4 p-4 p-md-5 backdrop-blur">
                        ...
                        <div class="card bg-dark bg-opacity-75 border-warning mb-4">
                            <div class="card-body">
                                ...
                                <p class="mb-2"><strong>Amount:</strong> Rp
                                    {{ number_format($duesPeriod->amount, 0, ',', '.') }}</p>
                                <p class="mb-2"><strong>Due Date:</strong> {{ $duesPeriod->due_date->format('d M Y') }}
                                </p>
                        ...
                        <form action="{{ route('dues.payment.store', $duesPeriod->id) }}" method="POST"
                            enctype="multipart/form-data">
                            @csrf

                            <div class="mb-4">
                                <label for="payment_proof" class="form-label fw-semibold">Upload Payment Proof <span
                                        class="text-danger">*</span></label>
                                <input type="file" name="payment_proof" id="payment_proof" accept="image/*" required
                                    class="form-control">
                                <small class="form-text text-white">Accepted formats: JPG, PNG (Max: 2MB)</small>
                            </div>
