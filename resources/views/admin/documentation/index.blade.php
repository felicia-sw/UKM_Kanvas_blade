...
@section('content')
    <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        <h1 class="h2 mb-0">Documentation for: {{ $event->title }}</h1>
        <div>
                    ...
                    <tbody>
                        @forelse ($documentations as $documentation)
                            <tr>
                                ...
                                <td>
                                    <a href="{{ route('admin.events.documentation.edit', ['event' => $event->id, 'documentation' => $documentation->id]) }}"
                                        class="btn btn-sm btn-admin-outline-warning me-1" title="Edit Media">
                                        <i class="bi bi-pencil"></i>
                                    </a>

                                    <form
                                        action="{{ route('admin.events.documentation.destroy', ['event' => $event->id, 'documentation' => $documentation->id]) }}"
                                        method="POST" class="d-inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="btn btn-sm btn-admin-outline-danger"
                                            title="Delete Media"
                                            onclick="return confirm('Are you sure you want to delete this media? This cannot be undone.')">
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="text-center">No documentation media has been uploaded for this
                                    event yet.</td>
                            </tr>
                        @endforelse
                    </tbody>
