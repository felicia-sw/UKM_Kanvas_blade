...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h1 class="h2">Edit Artwork: {{ $artwork->title }}</h1>
    ...
    <div class="card-body">
        ...
        <form action="{{ route('admin.artworks.update', $artwork->id)}}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control @error('artist_name') is-invalid @enderror" id="artist_name" name="artist_name" 
                       value="{{ old('artist_name', $artwork->artist_name) }}" required>
                @error('artist_name')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control @error('description') is-invalid @enderror" id="description" name="description" rows="4">{{ old('description', $artwork->description) }}</textarea>
                @error('description')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <div class="form-text">Accepted formats: JPG, PNG, GIF. Max size: 20MB. Leave blank to keep current image.</div>
                @error('image_file')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
