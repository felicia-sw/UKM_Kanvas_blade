            ...
            <div class="admin-card">
                <div class="card-body">
                    ...
                    @if($order->payment_proof)
                        <img src="{{ Storage::url($order->payment_proof) }}" alt="Payment Proof" class="img-fluid rounded mb-3">
                    @else
                    ...
                    @if($order->payment_status === 'pending')
                        <div class="d-flex gap-2 mt-3">
                            <form action="{{ route('admin.orders.verify-payment', $order->id) }}" method="POST" class="d-inline">
                                @csrf
                                @method('PATCH')
                ...
                <div class="row">
                    <div class="col-md-6">
                        <select name="pickup_status" class="form-select" required>
                            <option value="pending" {{ $order->pickup_status === 'pending' ? 'selected' : '' }}>Pending</option>
                            <option value="ready" {{ $order->pickup_status === 'ready' ? 'selected' : '' }}>Ready for Pickup</option>
                            <option value="picked_up" {{ $order->pickup_status === 'picked_up' ? 'selected' : '' }}>Picked Up</option>
                        </select>
                    </div>
                    <div class="col-md-6">
