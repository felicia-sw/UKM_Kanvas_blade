...
@section('content')
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-8">
                                ...
                                <div class="event-info mb-4">
                                    <p class="mb-2"><i class="bi bi-calendar3 me-2"></i><strong>Start:</strong>
                                        {{ $event->start_date->format('d M Y, H:i') }}</p>
                                    @if ($event->end_date)
                                        <p class="mb-2"><i class="bi bi-calendar3 me-2"></i><strong>End:</strong>
                                            {{ $event->end_date->format('d M Y, H:i') }}</p>
                                    @endif
                                    @if ($event->location)
                                        <p class="mb-2"><i class="bi bi-geo-alt me-2"></i><strong>Location:</strong>
                                            {{ $event->location }}</p>
                                    @endif
                                    @if ($event->registration_deadline)
                                        <p class="mb-2"><i class="bi bi-clock me-2"></i><strong>Registration
                                                Deadline:</strong> {{ $event->registration_deadline->format('d M Y') }}</p>
                                    @endif
                                    @if ($event->price)
                                        <p class="mb-2"><i class="bi bi-tag me-2"></i><strong>Price:</strong> Rp
                                ...
                                <div class="d-flex gap-3 flex-wrap">
                                    @if ($event->isPastEvent())
                                        <a href="{{ route('events.documentation', $event->id) }}"
                ...
                <div class="modal-dialog modal-lg modal-dialog-centered modal-dialog-scrollable">
                    <div class="modal-content bg-dark text-white">
                        ...
                        <div class="modal-body">
                            ...
                            <form action="{{ route('events.register', $event) }}" method="POST" enctype="multipart/form-data"
                                id="registrationForm">
                                @csrf
                                ...
                                <div class="mb-3">
                                    <label class="form-label fw-semibold">
                                        WhatsApp Number
                                        <span class="text-warning" title="Required for WhatsApp verification confirmation">
                                            <i class="bi bi-info-circle"></i>
                                        </span>
                                    </label>
                                    <input type="text" name="phone_number" id="phone_number"
                                        value="{{ old('phone_number', auth()->user()->profile?->no_telp ?? '') }}"
                                        placeholder="e.g., 081234567890"
                                        class="form-control @error('phone_number') is-invalid @enderror">
                                    <small class="form-text text-muted">
                                        <i class="bi bi-whatsapp me-1"></i>
                                        Enter your WhatsApp number to receive verification confirmation. Include country code
                                        (e.g., 081234567890)
                                    </small>
                                    @error('phone_number')
    ...
    <link rel="stylesheet" href="{{ asset('css/pages/event-show.css') }}">
